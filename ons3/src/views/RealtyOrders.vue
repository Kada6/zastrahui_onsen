<template>
  <div>
    ...
    <OrderPopup :hidePopup="hidePopup" :dialog="show_popup" :id='popup_id' :sendMessage="sendMessage" ></OrderPopup>
        ...
        <v-card :class="{'ma-0 xs12': $vuetify.breakpoint.smAndDown}"  mt-2>
            <v-container fluid text-xs-center>
              <v-layout column py-3 px-3>
                ...
                <v-flex xs4>
                  <v-btn round color="primary" large @click="showPopup(result.id)">Оформить за {{ result.insurance_price }} ₽</v-btn>
                </v-flex>
              </v-layout>
            </v-container>
          </v-card>
...
export default
{
  ...
  created()
  {
    this.$api.get(`/properties/` + this.$store.state.realty.id + `/info`)
      .then(response =>
      {
        ...
        {
          this.$api.get(`/properties/` + this.$store.state.realty.id + `/results`)
            .then(resp =>
            {
  ...
  methods:
  {
    ...
    {
      this.$api.post(`/properties/orders`,
        {
          name: name,
          phone: phone,
          property_results_id: id
        })
        .then(resp =>
        {
          //alert(resp.data)
